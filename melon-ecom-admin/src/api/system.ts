...
/** 轮播图管理-仅更新跳转配置 */
export const updateBannerJumpConfig = (
  id: number,
  jumpType: "NONE" | "PRODUCT" | "CATEGORY",
  jumpTargetId?: number | null
) => {
  const userData = getToken();
  const params: Record<string, string | number> = { jumpType };
  if (jumpTargetId != null) {
    params.jumpTargetId = jumpTargetId;
  }
  return http.request<Result>("patch", `/admin/updateBanner/${id}`, {
    headers: {
      Authorization: userData.accessToken
...
/** 钱包管理-查询用户余额 */
export const getUserWalletBalance = (userId: number) => {
  const userData = getToken();
  return http.request<Result>("get", `/admin/wallet/balance/${userId}`, {
    headers: { Authorization: userData.accessToken }
  });
};

/** 钱包管理-调整余额 */
export const adjustWallet = (data: {
  userId: number;
  amount: number;
  type: "ADD" | "SUB";
  remark?: string;
  ...
  const userData = getToken();
  return http.request<Result>("post", "/admin/wallet/adjust", {
    headers: { Authorization: userData.accessToken },
    data
  });
};
