...
import { ElInputNumber, ElSelect as ElSel, ElOption as ElOpt } from "element-plus";
import {
  ElForm,
  ElInput,
  ElFormItem,
...
export function useUser(tableRef: Ref) {
  ...
  async function onSearch() {
    try {
      ...
      if (result.code === 0 && result.data && result.data.records) {
        ...
        dataList.value = result.data.records.map(item => ({
          userId: item.userId,
  ...
  // ===== 钱包余额调整 =====
  function handleWallet(row) {
    const walletForm = reactive({ amount: 100, type: "ADD", remark: "" });
    const currentBalance = ref<number | null>(null);

    getUserWalletBalance(row.userId).then(res => {
      if (res.code === 0) currentBalance.value = Number(res.data);
    });

    addDialog({
      title: `调整余额 — ${row.username}（ID: ${row.userId}）`,
      width: "420px",
      draggable: true,
      closeOnClickModal: false,
      contentRenderer: () =>
        h("div", { class: "space-y-4 py-2" }, [
          h("div", { class: "text-sm text-gray-500 mb-2" }, [
            "当前余额：",
            h("span", { class: "font-semibold text-base text-gray-800" },
              currentBalance.value !== null ? `￥${currentBalance.value.toFixed(2)}` : "加载中…")
          ]),
          h("div", { class: "flex items-center gap-3" }, [
            h("span", { class: "w-16 text-sm shrink-0" }, "操作类型"),
            h(ElSel, {
              modelValue: walletForm.type,
              "onUpdate:modelValue": (v: string) => { walletForm.type = v; },
              class: "flex-1"
            }, () => [
              h(ElOpt, { value: "ADD", label: "充值（+）" }),
              h(ElOpt, { value: "SUB", label: "扣减（-）" })
            ])
          ]),
          h("div", { class: "flex items-center gap-3" }, [
            h("span", { class: "w-16 text-sm shrink-0" }, "金额"),
            h(ElInputNumber, {
              modelValue: walletForm.amount,
              "onUpdate:modelValue": (v: number) => { walletForm.amount = v; },
              min: 0.01, precision: 2, step: 10, class: "flex-1", style: "width:100%"
            })
          ]),
          h("div", { class: "flex items-center gap-3" }, [
            h("span", { class: "w-16 text-sm shrink-0" }, "备注"),
            h(ElInput, {
              modelValue: walletForm.remark,
              "onUpdate:modelValue": (v: string) => { walletForm.remark = v; },
              placeholder: "选填", clearable: true, class: "flex-1"
            })
          ])
        ]),
      beforeSure: async done => {
        if (!walletForm.amount || walletForm.amount <= 0) {
          message("金额必须大于 0", { type: "warning" });
          return;
        }
        const res = await adjustWallet({
          userId: row.userId,
          amount: walletForm.amount,
          type: walletForm.type as "ADD" | "SUB",
          remark: walletForm.remark
        });
        if (res.code === 0) {
          message(`操作成功，调整后余额 ￥${Number(res.data).toFixed(2)}`, { type: "success" });
          done();
        } else {
